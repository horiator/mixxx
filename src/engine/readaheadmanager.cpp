...
int ReadAheadManager::getNextSamples(double dRate, CSAMPLE* pOutput,
    ...
    int samples_from_reader = requested_samples;
    if (loop_active) {
        ...
        if (samplesToLoopTrigger < 0) {
            ...
            samples_from_reader = math_clamp(samples_from_reader, 0, preloop_samples);
        }
    }
    ...
    int samples_read = m_pReader->read(start_sample, in_reverse, samples_from_reader,
                                       pOutput);
    ...
    if (loop_active) {
        ...
        if (loop_target != kNoTrigger) {
            ...
            // e.g. exactly one bar.
            m_currentPosition = loop_target + overshoot;
